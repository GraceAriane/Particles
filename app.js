...
class Particule{
    ...
    dessine(){
        ctx.beginPath()
        //permet de dire qu'on commence un dessin
    ...
    MAJ(){
        if(this.x + this.taille > canvas.width || this.x - this.taille < 0){
            ...
            //on verifie ici si la particule a touché le bord du canva et on le redireige
        }
...
function animation(){
    ...
    //execute une animation 60fois par seconde
    ctx.clearRect(0,0,window.innerWidth, window.innerHeight)
    //on nettoie le rectangle
    for(let i = 0; i<particuleTab.length; i++){
        particuleTab[i].MAJ()
        //methode de maj qui va nous permettre de faire bouger les particules
    }
